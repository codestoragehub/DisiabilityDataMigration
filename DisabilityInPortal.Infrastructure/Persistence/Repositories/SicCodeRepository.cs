...
namespace DisabilityInPortal.Infrastructure.Persistence.Repositories
{
    ...
    {
        private readonly IRepositoryAsync<SicCode> _repository;

        public SicCodeRepository(IRepositoryAsync<SicCode> repository)
        {
            _repository = repository;
        }

        public Task<List<SicCode>> SearchAsync(string searchTerm)
        {
        ...
        public Task<List<SicCode>> GetListAsync(IEnumerable<string> sicCodes)
        {
            return _repository.Entities
                .OrderBy(c => c.Description)
                .Where(c => sicCodes.Contains(c.Code))
                .ToListAsync();
        }
    }
}